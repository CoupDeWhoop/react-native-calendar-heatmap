...
function getFillColor(index, valueCache, rectColor) {
  if (valueCache[index]) {
    const maxCount = Math.max(...valueCache.map(data => data.countedArray.count)); // Find the maximum count
    const fillColor = findColorLevel(valueCache[index].countedArray.count, rectColor, maxCount);
    return fillColor;
  }
  return rectColor[0];
}
